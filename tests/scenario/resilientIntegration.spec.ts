...
describe("Saga Engine End-to-End Integration - Resilient Path Resolution Flow", () => {
  ...
  it("should abort batch pre-flight and report ambiguous conflict suggestions when duplicates are found", async () => {
    ...
    const operations: AnyOperation[] = [
      {
        ...
        changes: [{ search: "content1", replace: "new" }],
        status: "pending",
      },
    ];
    ...
    assert.ok(
      log!.metadata?.conflict?.candidatePaths?.includes(
  ...
  it("should complete transactional Saga atomic rollback when nested execution steps fail", async () => {
    ...
    const operations: AnyOperation[] = [
        ...
        changes: [
          {
            ...
            replace: "export const StyledButton = () => {};",
          },
        ],
        status: "pending",
      },
      {
        ...
        changes: [
          {
            search: "NON_EXISTENT_MATH_FUNCTION_TRIGGERING_CONFLICT",
            replace: "sub",
          },
        ],
        status: "pending",
    ...
    assert.strictEqual(
      mockFilesystem.get(buttonUri),
      "export const Button = () => {};",
    );
    assert.strictEqual(
      mockFilesystem.get(mathUri),
      "export const add = (a, b) => a + b;",
    );
  });

  it("should execute create_file on an existing file as an overwrite and register restore_file compensation", async () => {
    const fileUriStr = "file:///workspace/src/components/Button.tsx";
    const originalContent = "export const Button = () => {};";
    mockFilesystem.set(fileUriStr, originalContent);

    const operations: AnyOperation[] = [
      {
        id: "op-overwrite-create",
        type: "create_file",
        path: "src/components/Button.tsx",
        content: "export const OverwrittenButton = () => {};",
        status: "pending",
      },
    ];
    ...
    // Verify the existing file was correctly overwritten on virtual disk
    assert.strictEqual(
      mockFilesystem.get(fileUriStr),
      "export const OverwrittenButton = () => {};",
    );

    // Revert changes
    await manager.revertOperation("op-overwrite-create");

    // Verify file has been successfully restored to its original state using snapshot
    assert.strictEqual(mockFilesystem.get(fileUriStr), originalContent);
  });
});
