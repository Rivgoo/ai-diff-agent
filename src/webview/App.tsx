...
// Clean, zero-dependency inline SVGs matching standard VS Code icons
const SettingsIcon = () => (
...
export const App = () => {
    ...
    const isTyping = useChatStore((state) => state.isAgentTyping);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: 'var(--vscode-sideBar-background)', position: 'relative' }}>
            ...
            {/* Header Area */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderBottom: '1px solid var(--vscode-panel-border)' }}>
                <span style={{ fontWeight: 'bold', color: 'var(--vscode-foreground)', fontSize: '12px' }}>AI Diff Engine</span>
            ...
            <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {messages.length === 0 ? (
                    <div style={{ textAlign: 'center', marginTop: '60px', color: 'var(--vscode-descriptionForeground)' }}>
                        <span style={{ fontSize: '48px', marginBottom: '16px', display: 'block' }}>🤖</span>
                        Agent is ready. Paste your AI XML payload below.
                    </div>
                ) : (
                    messages.map((msg) => (
                ...
                {isTyping && (
                    <div style={{ color: 'var(--vscode-descriptionForeground)', fontStyle: 'italic', paddingLeft: '4px' }}>
                        Agent is thinking...
                    </div>
                )}
            </div>
