...
export class TransactionManager {
  ...
  public async applyBatch(operations: AnyOperation[]): Promise<void> {
    ...
    const pendingOpIds = operations
      .filter((op) => op.status === "pending")
      .map((op) => op.id);
    if (!this.transactionLock.acquireBatch(pendingOpIds)) {
      OutputLogger.log(
        "Transaction rejected. Lock held by other operations.",
        "WARN",
      );
    ...
    if (!preFlightResult.success) {
      OutputLogger.log(
        `Pre-flight checks failed: ${preFlightResult.failReason}`,
        "ERROR",
      );
      this.abortBatch(
        operations,
        preFlightResult.failReason,
        preFlightResult.conflictMap,
      );
      return;
    }
    ...
    await this.executeModifications(
      ...
      rootUri,
      rootName,
      preFlightResult.opCandidatesMap,
    );
  }
    ...
    for (const op of operations) {
      ...
      try {
        ...
        if (resolution.status === "AMBIGUOUS_MATCH") {
          ...
          return {
            success: false,
            failReason: "Ambiguous matches found.",
            conflictMap,
            opCandidatesMap,
          };
        }
        ...
        if (
          ...
          (isUpdateFileOperation(op) ||
            isDeletePathOperation(op) ||
            isMovePathOperation(op))
        ) {
          conflictMap.set(op.id, this.buildConflict("FILE_NOT_FOUND", op));
          return {
            success: false,
            failReason: `File not found: ${normalizedPath}`,
            conflictMap,
            opCandidatesMap,
          };
        }
        ...
        return {
          success: false,
          failReason: "Path traversal validation failed.",
          conflictMap,
          opCandidatesMap,
        };
      }
      ...
      if (isUpdateFileOperation(op)) {
        ...
        for (let i = 0; i < op.changes.length; i++) {
          ...
          if (match.status !== "MATCHED") {
            const reasonCode: ConflictReason =
            ...
            return {
              success: false,
              failReason: `Search match failed. Reason: ${match.reason}`,
              conflictMap,
              opCandidatesMap,
            };
          }
        ...
        try {
          ...
          return {
            success: false,
            failReason: `Source file not found: ${targetUri.fsPath}`,
            conflictMap,
            opCandidatesMap,
          };
        }
  ...
  private async executeModifications(
    ...
    const pendingDecorations = new Map<
      string,
      { uri: vscode.Uri; ranges: vscode.Range[] }
    >();
    ...
    try {
      for (const op of operations) {
        ...
        if (
          isCreateFileOperation(op) ||
          isMovePathOperation(op) ||
          isCreateDirOperation(op)
        ) {
          const targetPath = isMovePathOperation(op)
            ? op.destinationPath
            : op.path;
          const normalized = PathNormalizer.normalize(targetPath, rootName);
          const targetUri = PathSandbox.validate(normalized);
          const parentDir = isCreateDirOperation(op)
            ? targetUri
      ...
      for (const op of operations) {
        ...
        if (isCreateFileOperation(op)) {
          ...
          if (fileExists) {
            // File already exists on disk. Re-route the workflow to an overwrite strategy and capture a backup snapshot.
            ...
            // Retrieve the document reference to safely calculate its full range
            ...
            edit.replace(targetUri, fullRange, op.content);
            antiActions.push({
              type: "restore_file",
              uri: targetUri,
            ...
            pendingDecorations.set(op.id, {
              ...
              ranges: [
                new vscode.Range(
                  ...
                  new vscode.Position(lineCount, 999),
                ),
              ],
            });
          } else {
            // File does not exist on disk. Proceed with standard file creation.
          ...
          const normalizedDest = PathNormalizer.normalize(
            op.destinationPath,
            rootName,
          );
          ...
          for (const match of matchedBlocks) {
            ...
            appliedRanges.push(
              new vscode.Range(
                match.range.start.line,
                0,
                match.range.start.line + lineDelta - 1,
                999,
              ),
            );
          }
      ...
      const cleanedDirs =
        await this.directoryCleanupService.cleanupEmptyDirectories(
          allCandidates,
          rootUri,
        );
      ...
      for (const dirUri of cleanedDirs) {
        ...
        for (const [opId, candidates] of opCandidatesMap.entries()) {
          if (candidates.includes(relativeDir)) {
            opAntiActionsMap
              .get(opId)
              ?.push({ type: "restore_dir", uri: dirUri });
            break;
          }
        }
      }
      ...
      for (const op of operations) {
        ...
        if (antiActions.length > 0) {
          ...
          if (pending) {
            this.decorationService.addDecorations(
              pending.uri,
              op.id,
              pending.ranges,
            );
      ...
      OutputLogger.log(
        `Failed to execute workspace modifications: ${err}`,
        "ERROR",
      );
      ...
      for (const op of operations) {
        ...
        if (op.status === "pending") {
          op.errorMessage =
            err instanceof Error ? err.message : "Unknown write/edit error";
  ...
  private abortBatch(
    ...
    for (const op of operations) {
      ...
      if (op.status === "pending") {
        ...
        op.errorMessage = conflictData
          ? failReason
          : "Transaction aborted due to conflict in other files.";
  ...
  public async saveOperation(opId: string): Promise<void> {
    ...
    await this.snapshotService.purgeSnapshotForOp(
  ...
  public async revertOperation(opId: string): Promise<void> {
    ...
    for (const act of tx.antiActions) {
      if (act.type === "restore_file")
        await this.snapshotService.restoreSnapshot(
          rootUri,
          opId,
          act.relativePath,
          act.uri,
  ...
  private async trackAndCreateDirectory(
    rootUri: vscode.Uri,
